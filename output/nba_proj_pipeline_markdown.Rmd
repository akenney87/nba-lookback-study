...
params:
  per36_csv: "Per36Minutes.csv"
  career_csv: "PlayerCareerInfo.csv"
  target_season: 2026
editor_options:
...
# Convenience: resolve paths via here::here() so knitting from RStudio just works
per36_path  <- here::here(params$per36_csv)
career_path <- here::here(params$career_csv)
```
...
raw <- readr::read_csv(per36_path, show_col_types = FALSE)
...
by_player_season <- raw %>%
  ...
  summarise({
    ...
    tibble::tibble(
      player = df$player[idx_mm], pos = df$pos[idx_mm],
...
lagged_list <- lapply(1:5, function(i) {
  base %>% rename_with(~ paste0(.x, "_L", i), all_of(stats_vec)) %>% mutate(season = season + i)
})
...
predict_equal_k <- function(df, stat, k) {
  ...
  df %>%
    ...
    mutate(pred_equal = rowMeans(across(all_of(lag_cols)))) %>%
...
fit_and_predict_reg_k <- function(df, stat, k, train_end_season) {
  ...
  test %>% mutate(pred_lm = predict(fit, newdata = test)) %>% select(player_id, season, actual = !!stat, pred_lm)
}
...
knitr::kable(winners, caption = "Best method per stat (through Step 3).")
```

```{r predict 2026 per-36 with graceful fallback and year over year deltas}
# Experience year from career "from"
career <- readr::read_csv(career_path, show_col_types = FALSE) %>%
  janitor::clean_names() %>%
